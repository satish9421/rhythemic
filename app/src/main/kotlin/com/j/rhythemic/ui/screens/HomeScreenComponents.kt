 ...
 * │  Signature: Rhythemic::UI::YTM_PREMIUM_V2     │
 * ╰────────────────────────────────────────────╯
 */
...
import com.j.rhythemic.models.SimilarRecommendation
...
import com.j.rhythemic.viewmodels.CommunityPlaylistItem
...
fun KeepListeningSection(
    ...
    LazyHorizontalGrid(
        ...
        items(
            ...
            key = { item -> 
                when (item) { 
                    is com.j.rhythemic.db.entities.Song -> "song_${item.id}"
                    is com.j.rhythemic.db.entities.Album -> "album_${item.id}"
                    is com.j.rhythemic.db.entities.Artist -> "artist_${item.id}"
                    is com.j.rhythemic.db.entities.Playlist -> "playlist_${item.id}"
                    else -> "local_${item.id}"
                } 
            }
        ) { item ->
...
private fun LocalGridItem(
    ...
    when (item) {
        is com.j.rhythemic.db.entities.Song -> SongGridItem(
            song = item,
        ...
        is com.j.rhythemic.db.entities.Album -> AlbumGridItem(
            album = item,
        ...
        is com.j.rhythemic.db.entities.Artist -> ArtistGridItem(
            artist = item,
            modifier = modifier.fillMaxWidth().combinedClickable(
        ...
        is com.j.rhythemic.db.entities.Playlist -> {}
        else -> {}
    }
}
...
fun SimilarRecommendationsTitle(
    ...
    NavigationTitle(
        ...
        thumbnail = recommendation.title.thumbnailUrl?.let { thumbnailUrl ->
            {
                val shape = if (recommendation.title is com.j.rhythemic.db.entities.Artist) CircleShape else RoundedCornerShape(ThumbnailCornerRadius)
        ...
        onClick = {
            ...
            when (itemTitle) {
                is com.j.rhythemic.db.entities.Song -> navController.navigate("album/${itemTitle.album!!.id}")
                is com.j.rhythemic.db.entities.Album -> navController.navigate("album/${itemTitle.id}")
                is com.j.rhythemic.db.entities.Artist -> navController.navigate("artist/${itemTitle.id}")
                is com.j.rhythemic.db.entities.Playlist -> {}
                else -> {}
            }
        },
