    ...
    suspend fun submitPlayingNow(context: Context, token: String, song: Song?, positionMs: Long): Boolean {
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val trackMetadata = "{\"track_metadata\":{\"artist_name\":\"${escapeJson(artistNames)}\",\"track_name\":\"${escapeJson(song.title)}\",${releasePart}\"additional_info\":{\"duration_ms\":$durationMs,\"position_ms\":$positionMs,\"submission_client\":\"Rhythemic\"}}}"
    ...
    suspend fun submitFinished(context: Context, token: String, song: Song?, startMs: Long, endMs: Long): Boolean {
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                val trackMetadataSingle = "{\"listened_at\":$listenedAtStart,\"track_metadata\":{\"artist_name\":\"${escapeJson(artistNames)}\",\"track_name\":\"${escapeJson(song.title)}\",${releasePart}\"additional_info\":{\"duration_ms\":$durationMs,\"start_ms\":$startMs,\"end_ms\":$endMs,\"submission_client\":\"Rhythemic\"}}}"
