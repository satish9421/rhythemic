...
import com.j.rhythemic.constants.QuickPicks
import com.j.rhythemic.constants.QuickPicksKey
import com.j.rhythemic.constants.YtmSyncKey
import com.j.rhythemic.db.MusicDatabase
import com.j.rhythemic.db.entities.*
                    ...
                    val available = localItems.filter { item ->
                        ...
                        when (item) {
                            is Song -> SongItem(
                                id = item.id,
                                title = item.title,
                                artists = item.artists.map { com.j.rhythemic.innertube.models.Artist(name = it.name, id = it.id) },
            ...
            .map { song ->
                SongItem(
                    ...
                    artists = song.artists.map {
                        com.j.rhythemic.innertube.models.Artist(
                            name = it.name,
                            id = it.id
                        )
                    },
            ...
            quickPicks.value?.let { songs ->
                userSongs.addAll(songs.map { song ->
                    SongItem(
                        id = song.id,
                        title = song.title,
                        artists = song.artists.map { com.j.rhythemic.innertube.models.Artist(name = it.name, id = it.id) },
                        thumbnail = song.thumbnailUrl ?: "",
                        explicit = song.song.explicit,
                    )
                })
